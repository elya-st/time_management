...
class InterfaceTM:
    ...
    def __init__(self, notes_facade, tasks_facade, dml):
        ...
        self.__menu_map = {
            ...
            "9": functools.partial(interface_common.quit_program,
                                   self.notes_facade)
        }
    ...
    def __complete_task(self, first_run=True):
        ...
        InterfaceTM.__print_entries(formatted_tasks)
        if first_run:
            task_input = input("Enter space delimited task IDs for "
                               "completion ")
        else:
            task_input = input("One or more tasks IDs not found.Please try "
                               "again: ")
    ...
    def __void_task(self, first_run=True):
        ...
        print("0 to cancel\n")
        if first_run:
            task_input = input("Enter space delimeted task IDs voiding task: ")
        else:
            task_input = input("One or more task IDs not found.Please try "
                               "again: ")
        task_ids = self.__split_tasks(task_input)
        ...
        if task_ids[0] == "0":
            ...
            for task_id in task_ids:
                if self.tasks_facade.check_if_not_voided(task_id):
                    ...
                    self.notes_facade.insert_note("Voided Task" + task_id +
                                                  ": " + task)
                else:
                    print(f"You have already Voided this task with task id - "
                          f"{task_id}")
                    print("please wait !!")
                    time.sleep(3)
    ...
    def __print_overdue(self):
        interface_common.clear_screen()
        task_rows = self.tasks_facade.get_overdue_task()
        formatted_tasks = format_decorators.format_task(task_rows)
    ...
    def __print_scrum_notes(self):
        ...
        interface_common.parse_ascii_banner(interface_common.parse_ascii_banner(
            banner
        ))
